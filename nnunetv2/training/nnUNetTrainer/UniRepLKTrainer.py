...
class UniRepLKTrainer(nnUNetTrainer):
    ...
    def configure_optimizers(self):
        ...
        def lr_decay_func(param_name: str, base_lr: float, decay_rate: float) -> float:
            if 'conv_blocks_context.4.0' in param_name:  # Last stage
                return base_lr * decay_rate
            elif 'conv_blocks_context.3.0' in param_name:  # Second last stage
                return base_lr * (decay_rate ** 2)
            elif 'conv_blocks_context.2.0' in param_name:  # Third last stage
                return base_lr * (decay_rate ** 3)
            elif 'conv_blocks_context.1.0' in param_name:  # First stage
                return base_lr * (decay_rate ** 4)
            elif 'conv_blocks_context.0.0' in param_name:  # First stage
                return base_lr * (decay_rate ** 5)
            else:  # Non-pretrained parameters or other layers
                return base_lr

        decay_rate = 0.8
        param_groups = []
        for name, param in self.network.named_parameters():
            lr = lr_decay_func(name, self.initial_lr, decay_rate)
            param_groups.append({'params': param, 'lr': lr, 'weight_decay': self.weight_decay})
...
class UniRepLKTrainer_base(UniRepLKTrainer):
    ...
    def build_network_architecture(plans_manager,
        ...
        return UniRepLKUNet(num_input_channels, num_classes, depth=[1] * 6, dims=[32 * x for x in [1, 2, 4, 8, 16, 16]],
                      ...
                      enable_deep_supervision=enable_deep_supervision, drop_path_rate=drop, layer_scale_init_value=layer_scale)

class UniRepLKTrainer_base_hybridv2_ft_boot(UniRepLKTrainer_base):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)


class UniRepLKTrainer_base_hybridv2_ft(UniRepLKTrainer_base):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
...
class UniRepLKTrainer_large(UniRepLKTrainer):
    ...
    def build_network_architecture(plans_manager,
        ...
        return UniRepLKUNet(num_input_channels, num_classes, depth=[2] * 6, dims=[64 * x for x in [1, 2, 4, 8, 16, 16]],
                            LK_kernels=[[3,3],[3,3],
                                      [13,3],
                                      [13,3],[3,3]],
                            pool_op_kernel_sizes=strides, conv_kernel_sizes=kernel_sizes,
...
class UniRepLKUNet(nn.Module):
    def __init__(self, input_channels, num_classes,LK_kernels,depth=[2,2,2,2,2,2], dims=[1, 2, 4, 8, 16,16]*48,
        ...
        dp_rates = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depth[1:5]))]
...
def fuse_in(conv, inst_norm):
    ...
    return conv.weight * inst_norm.weight.reshape(-1, 1, 1, 1, 1), (conv_bias) + inst_norm.bias
...
if __name__ == '__main__':
    ...
    model = UniRepLKUNet(1,105, depth=[2] * 6, dims=[64 * x for x in [1, 2, 4, 8, 16, 16]],
                                LK_kernels=[[3,3],[3,3],
                                          [13,3],
                                          [13,3],[3,3]], pool_op_kernel_sizes=strides, conv_kernel_sizes= [[3, 3, 3]] * 6,
                          enable_deep_supervision=True, drop_path_rate=0.2, layer_scale_init_value=1e-6).to(device)
    ...
    # for j in s1:
    #     print(j.shape)
    # try to reparametrize

    # print('looking to reparam: ')
    ...
    #
    # weights = torch.load('/home/yl_li/STUNet/nnUNet-1.7.1/nnUNet_results/Dataset501_Total/UniRepLKTrainer_base__nnUNetPlans__3d_fullres/fold_0/checkpoint_best.pth')
    #
    #
