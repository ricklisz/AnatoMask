...
class SparK(nn.Module):
    def __init__(
            self, sparse_encoder: encoder3D.SparseEncoder, dense_decoder: LightDecoder, loss_decoder: LightDecoder,
    ...
    @torch.no_grad()
    def generate_mask(self, loss_pred, guide = True, epoch = 0, total_epoch = 200, generator=None):
        h, w, d= self.fmap_h, self.fmap_w, self.fmap_d
        B, L = loss_pred.shape
    ...
    def forward(self, inp_bchwd: torch.Tensor, get_feature = False, active_b1ff=None, vis=False):
        # step1. Mask
        ...
        feat_bchwd = self.loss_decoder(to_dec[0])
        ...
        feat_patch = self.patchify_feat(feat_bchwd)
        ...
        if vis:
            if self.use_hog:
                ...
                return inp_bchwd, masked_bchwd, feat_bchwd, rec_or_inp

        else:

            return inp, rec, feat_patch

    def forward_loss(self, inp, rec, active_b1ff):

        # is_all_zero = (active_b1ff.sum() == 0)
        # if is_all_zero:
        #     print("The mask is all zero.")
        # is_all_one = (active_b1ff.sum() == active_b1ff.numel())
        # if is_all_one:
        #     print("The mask is all one.")
