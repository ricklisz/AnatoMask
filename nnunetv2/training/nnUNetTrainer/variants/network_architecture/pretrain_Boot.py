...
from spark3D_Boot import SparK
...
head = UniRepLKUNet(1, 1, depth=[1] * 6, dims=[32 * x for x in [1, 2, 4, 8, 16, 16]],
                     LK_kernels=[[3], [3],
                                 [13],
                                 [13], [3]], pool_op_kernel_sizes=pool_op_kernel_sizes, conv_kernel_sizes=[[3, 3, 3]] * 6,
                     enable_deep_supervision=True).to(device)
...
class FCDecoder(torch.nn.Module):
    def __init__(self, in_channels=512, out_channels=512):
        ...
        self.block = torch.nn.Sequential(
            torch.nn.Conv3d(in_channels*4, in_channels*4, kernel_size=3, stride=1, padding=1, bias=False),
            torch.nn.InstanceNorm3d(in_channels*4), torch.nn.ReLU6(inplace=True),
            torch.nn.Conv3d(in_channels*4, out_channels, kernel_size=3, stride=1, padding=1, bias=False),
...
class DualFCDecoder(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        super(DualFCDecoder, self).__init__()
        self.decoder1 = FCDecoder(in_channels=in_channels, out_channels=in_channels)
        self.decoder2 = FCDecoder(in_channels=in_channels, out_channels=out_channels)

    def forward(self, x):
        # Example forward pass, adjust according to your needs
        x1 = self.decoder1(x)
        x2 = self.decoder2(x1)
        # You can then combine x1 and x2 or return them as needed
        return x2
...
if use_hog:
    ...
    loss_dec = FCDecoder(in_channels = 512, out_channels = 512).to(device)
    # loss_dec = DualFCDecoder(in_channels = 512, out_channels = 512).to(device)
...
# checkpoint = torch.load('/home/yl_li/STUNet/nnUNet-1.7.1/nnUNet_results/Dataset501_Total/Pretraining/UniRep_B_Boot/UniRep_B_head_latest.pt')
...
output_folder = '/home/yl_li/STUNet/nnUNet-1.7.1/nnUNet_results/Dataset501_Total/Pretraining/' + model_name +'_Boot'
...
for i in range(epoch):
    ...
    # feature_weight = fw * (1+i) / epoch
    # feature_weight = min(feature_weight*4, fw)
    feature_weight = 0.2
    ...
    for idx in range(iters_train):
        ...
        if AMP:
            ...
            mask1 = (torch.rand((batch_size, 1, 7, 7, 8), device=device) > -1).bool()
            if model_ema is not None:
                with torch.no_grad():
                    _, _, LK_feat = model_ema.ema(inp, get_feature=True, active_b1ff=mask1)
            mask2 = (torch.rand((batch_size, 1, 7, 7, 8), device=device) > 0.6).bool()
            inpp, recc, feat_pred = model(inp, active_b1ff=mask2, vis=False)
            # loss_p = model.module.forward_loss(inpp, recc, mask2)
            loss_feat = model.module.forward_loss(
                LK_feat.detach(), feat_pred,
                mask2
            )
            # loss = loss_p + feature_weight*loss_feat
            loss = loss_feat
            optimizer.zero_grad()
            loss.backward()
            # optimize
