...
class SparK(nn.Module):
    def __init__(
            self, sparse_encoder: encoder3D.SparseEncoder, dense_decoder: DSDecoder,
        ...
        self.register_buffer('imn_m', torch.empty(1, 3, 1, 1, 1))
        self.register_buffer('imn_s', torch.empty(1, 3, 1, 1, 1))
        self.register_buffer('norm_black', torch.zeros(1, 3, input_size[0], input_size[1], input_size[2]))
    ...
    def forward(self, inp_bchwd: torch.Tensor, active_b1ff=None, vis=False):
        ...
        downsampled_list = [inp_bchwd]
        current_tensor = inp_bchwd

        for _ in range(3):
            current_tensor = F.interpolate(current_tensor, scale_factor=0.5, mode='trilinear', align_corners=False)
            downsampled_list.append(current_tensor)
        ...
        ds_rec = self.dense_decoder(to_dec)
        ds_rec.reverse()

        recon_loss = []
        for i in range(len(ds_rec)):
            inp,  rec = self.patchify(downsampled_list[i], 2**(len(ds_rec)-i)), self.patchify(ds_rec[i], 2**(len(ds_rec)-i))  # inp and rec: (B, L = f*f*f, N = C*downsample_raito**3)
            ...
            recon_loss.append(l2_loss.mul_(non_active).sum() / (
                    non_active.sum() + 1e-8))  # loss only on masked (non-active) patches

        if vis:
            # Assuming inp_bchwd is a representation of the downsampled input, hence using downsampled_list[0]
            inp_patch = self.patchify(downsampled_list[0], 2 ** (len(ds_rec)))

            # Normalize inp_bchwd
            mean = inp_patch.mean(dim=-1, keepdim=True)
            var = (inp_patch.var(dim=-1, keepdim=True) + 1e-6) ** .5
            # Get the reconstruction based on ds_rec[0] and adjust it
            rec_bchwd = self.unpatchify(self.patchify(ds_rec[0], 2 ** (len(ds_rec))) * var + mean)
            # Blend the input and its reconstruction based on the active mask
            rec_or_inp = torch.where(active_b1hwd, inp_bchwd, rec_bchwd)

            return inp_bchwd, inp_bchwd * active_b1hwd, rec_or_inp
        else:
            return recon_loss

    def patchify(self, bchwd, p):
        B, C = bchwd.shape[:2]
        h, w, d = bchwd.shape[2]//p, bchwd.shape[3]//p, bchwd.shape[4]//p
