...
from decoder3D import SegDecoder, LightDecoder, SMiMTwoDecoder, SMiMDecoder
...
def averaged_reconstruction(mask1, mask2, rec1, rec2):
    """
    Optimized function to merge two reconstruction images based on overlapping masked regions,
    adapted for inputs where masks are of shape (B, L) and reconstructions are of shape (B, L, C).

    Parameters:
    - mask1, mask2: Binary masks of shape (B, L) where 0s represent masked and 1s represent unmasked areas.
    - rec1, rec2: Reconstruction images of shape (B, L, C) corresponding to mask1 and mask2, respectively.

    Returns:
    - Optimized merged reconstruction image of shape (B, L, C).
    """
    # Ensure masks are correctly broadcasted over the channels
    mask1_expanded = mask1[:, :, np.newaxis]
    mask2_expanded = mask2[:, :, np.newaxis]

    # Initialize merged reconstruction with the same shape as the reconstructions
    merged_recon = torch.zeros_like(rec1)

    # Conditions adapted for expanded masks
    both_masked = ~(mask1_expanded | mask2_expanded)  # Both are 0
    mask1_masked_mask2_not = ~mask1_expanded & mask2_expanded  # Mask1 is 0 and Mask2 is 1
    mask2_masked_mask1_not = mask1_expanded & ~mask2_expanded  # Mask2 is 0 and Mask1 is 1

    # Apply averaging and filling based on the conditions
    merged_recon[both_masked] = (rec1[both_masked] + rec2[both_masked]) / 2
    merged_recon[mask1_masked_mask2_not] = rec2[mask1_masked_mask2_not]
    merged_recon[mask2_masked_mask1_not] = rec1[mask2_masked_mask1_not]
...
if use_hog:
    ...
    loss_dec = LightDecoder(enc.downsample_ratio,sbn=False, width = 512, use_IN= True, out_channel = 1).to(device)
    dec = SegDecoder(enc.downsample_ratio,sbn=False, width = 512, out_channel = 1).to(device)
...
output_folder = '/home/yl_li/STUNet/nnUNet-1.7.1/nnUNet_results/Dataset501_Total/Pretraining/' + model_name +'_HPM_no_dec_add_NCC_asymmetric_90_seg_dec'
...
for i in range(epoch):
    ...
    for idx in range(iters_train):
        ...
        if AMP:
            ...
            mask1 = (torch.rand((batch_size, 1, 7, 7, 8), device=device) > 0.9).bool()
            ...
            if model_ema is not None:
                with torch.no_grad():
                    ...
                    non_active = mask1.logical_not().int().view(mask1.shape[0], -1)
                    recon_loss = l2_loss * non_active
                    full_rec = model.module.unpatchify(rec1)
                    down_inp = torch.nn.functional.interpolate(inp, scale_factor=0.5, mode='trilinear',
                                                            align_corners=True)
                    ssim = calculate_3d_ssim_per_patch(down_inp, full_rec, patch_size= (8,8,8))
        ...
        with torch.no_grad():
            ...
            for idx in range(iters_val):
                ...
                if AMP:
                    ...
                    #         if use_ema:
                    #             if i == 0:
                    #                 ema_loss_vector_val = recon_loss
                    #             else:
                    #                 # Update EMA loss vector
                    #                 ema_loss_vector_val = alpha * ema_loss_vector_val + (1 - alpha) * recon_loss
                    #
                    # if use_ema:
                    # mask, easy_mask = model_ema.ema.generate_mask(recon_loss, guide=guide, epoch=i,total_epoch=epoch - 1)
                    mask, easy_mask = model_ema.ema.generate_mask(my_score, guide=guide, epoch=i,total_epoch=epoch - 1)
