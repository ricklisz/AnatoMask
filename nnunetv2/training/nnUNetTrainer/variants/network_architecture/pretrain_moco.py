...
def get_training_transforms2(patch_size: Union[np.ndarray, Tuple[int]],
                            rotation_for_DA: dict,
                            deep_supervision_scales: Union[List, Tuple],
                            mirror_axes: Tuple[int, ...],
                            do_dummy_2d_data_aug: bool,
    ...
    tr_transforms.append(GaussianBlurTransform((0.5, 1.), different_sigma_per_channel=True, p_per_sample=0.1,
                                               p_per_channel=0.5))
    tr_transforms.append(BrightnessMultiplicativeTransform(multiplier_range=(0.75, 1.25), p_per_sample=0.25))
    tr_transforms.append(ContrastAugmentationTransform(p_per_sample=0.15))
    tr_transforms.append(SimulateLowResolutionTransform(zoom_range=(0.5, 1), per_channel=True,
                                                        p_per_channel=0.5,
                                                        order_downsample=0, order_upsample=3, p_per_sample=0.2,
                                                        ignore_axes=ignore_axes))
    tr_transforms.append(GammaTransform((0.7, 1.5), True, True, retain_stats=True, p_per_sample=0.15))
    tr_transforms.append(GammaTransform((0.7, 1.5), False, True, retain_stats=True, p_per_sample=0.3))
...
class MoCo(nn.Module):
    def __init__(self, dim=128, K=65536, m=0.99, T=0.14):
        '''
        ...
        self.encoder_q = nn.Sequential(
            STUNet(1,1,depth=[1, 1, 1, 1, 1, 1], dims=[32, 64, 128, 256, 512, 512], pool_op_kernel_sizes = pool_op_kernel_sizes, conv_kernel_sizes = conv_kernel_sizes,
    ...
    def _dequeue_and_enqueue(self, keys):
        ...
        ptr = int(self.queue_ptr)
        assert self.K % batch_size == 0  # Ensure queue size is divisible by batch size

        # Calculate the space left at the end of the queue
        space_left = self.K - ptr

        if batch_size <= space_left:
            # If the entire batch fits in the remaining space
            self.queue[:, ptr:ptr + batch_size] = keys.T
            ptr = (ptr + batch_size) % self.K
        else:
            # If the batch needs to be split
            end = batch_size - space_left
            self.queue[:, ptr:] = keys.T[:, :space_left]  # Fill the end of the queue
            self.queue[:, :end] = keys.T[:, space_left:]  # Wrap around to the beginning
            ptr = end
...
model = MoCo(512).to(device)

print(model)

logits, labels = model(d,d)
...
tr_transforms2 = get_training_transforms2(
    patch_size, rotation_for_DA, deep_supervision_scales, mirror_axes, False,
    order_resampling_data=3, order_resampling_seg=1,
    use_mask_for_norm=configuration_manager.use_mask_for_norm,
    is_cascaded=False, foreground_labels=label_manager.foreground_labels,
...
mt_gen_train_target = LimitedLenWrapper(iters_train, data_loader=dl_tr, transform=tr_transforms,
                                 num_processes=allowed_num_processes, num_cached=6, seeds=[1] * allowed_num_processes,
                                 pin_memory= True, wait_time=0.02)

mt_gen_train_online = LimitedLenWrapper(iters_train, data_loader=dl_tr, transform=tr_transforms2,
                                 num_processes=allowed_num_processes, num_cached=6, seeds=[1] * allowed_num_processes,
                                 pin_memory= True, wait_time=0.02)
...
# #
# model.load_state_dict(torch.load( output_folder + '/STUNet_B_head_latest.pt')['network_weights'])
# optimizer.load_state_dict(torch.load( output_folder + '/STUNet_B_head_latest.pt')['optimizer_state'])
...
for i in range(epoch):
    ...
    for idx in range(iters_train):
        inp1 = next(mt_gen_train_target)

        inp1 = inp1['data']
        inp1 = inp1.to(device, non_blocking=True)

        inp2 = next(mt_gen_train_online)

        inp2 = inp2['data']
        inp2 = inp2.to(device, non_blocking=True)

        output, target = model(inp1, inp2)
...
#
# slice_num = 64
# inp1_bchw = inp1[:,:, slice_num,:,:]
# inp2_bchw = inp2[:,:, slice_num,:,:]
#
#
# plt.figure()
# for col, (title, bchw) in enumerate(zip(['input1', 'inp2'], [inp1_bchw, inp2_bchw])):
#     plt.subplot2grid((1, 4), (0, col))
#     plt.imshow(bchw[0,0].cpu().numpy(), plt.cm.gray)
#     plt.title(title)
#     plt.axis('off')
# plt.show()
# plt.savefig('test_pair.png')
# plt.close()
#
# print('Finished')
#
# # # build optimizer and lr_scheduler
#
# optimizer = torch.optim.AdamW(lr = lr, weight_decay= weight_decay, params = model_target.parameters())
# # print(f'[optimizer] optimizer({opt_clz}) ={optimizer}\n')
# # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, epoch)
# scheduler = LinearWarmupCosineAnnealingLR(optimizer, warmup, epoch, 1e-6)
#
