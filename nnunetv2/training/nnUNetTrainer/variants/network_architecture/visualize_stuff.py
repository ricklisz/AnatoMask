...
# checkpoint = torch.load('/home/yl_li/STUNet/nnUNet-1.7.1/nnUNet_results/Dataset501_Total/Pretraining/STUNet_B_HOG/STUNet_B_head_best.pt')
...
checkpoint = torch.load('/home/yl_li/STUNet/nnUNet-1.7.1/nnUNet_results/Dataset501_Total/Pretraining/UniRep_B/UniRep_B_head_best.pt')
...
head = UniRepLKUNet(1,1,depth=[1] * 6, dims=[32 * x for x in [1, 2, 4, 8, 16, 16]],
                        LK_kernels=[[3],[3],
                                      [13],
                                      [13],[3]],pool_op_kernel_sizes = pool_op_kernel_sizes, conv_kernel_sizes = conv_kernel_sizes,
              enable_deep_supervision=True).to(device)
...
active_b1ff = (torch.rand((batch_size,1,7,7,8), device=device) > mask_ratio).bool()
...
# inp, hogged, masked, rec_or_inp = model(img1, active_b1ff=active_b1ff, vis=True)
...
# rec_or_inp = rec_or_inp[:,hog_chan,:,:].unsqueeze(1)
