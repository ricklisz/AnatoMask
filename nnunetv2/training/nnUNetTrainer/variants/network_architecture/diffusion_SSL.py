...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, y=None):
        ...
        # the above h is the encoder output
...
if __name__ == '__main__':
    device = th.device(type='cuda', index=4)
    ...
    num_heads = [4, 4, 8, 8, 8, 16, 16]
    window_size = [[4, 4, 4], [4, 4, 2], [4, 4, 1], [4, 4, 1], [4, 4, 1], [4, 4, 1], [4, 4, 1]]
    num_res_blocks = [2, 2, 2, 2, 2, 2, 2]
    sample_kernel = ([2, 2, 2], [2, 2, 1], [2, 2, 1], [2, 2, 1], [2, 2, 1], [2, 2, 1]),
    ...
    device = 'cuda:0'
    x = th.ones(2, 1, 256, 256, 4)
    t, weights = schedule_sampler.sample(x.shape[0], device)

    noise = th.randn_like(x)

    noisy_image = diffusion.q_sample(x, t, noise)