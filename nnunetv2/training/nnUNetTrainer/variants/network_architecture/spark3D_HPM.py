...
class SparK(nn.Module):
    def __init__(
            self, sparse_encoder: encoder3D.SparseEncoder, dense_decoder: LightDecoder, loss_decoder=None,
    ...
    def generate_mask(self, loss_pred, guide = True, epoch = 0, total_epoch = 200, generator=None, original_mask = None):
        ...
        if guide:
            ...
            # k=0.01
            # keep_ratio = 0.5/ (1 + np.exp(-k * ((epoch + 1) - total_epoch//2)))
            ### hard-to-easy
    ...
    def forward(self, inp_bchwd: torch.Tensor, active_b1ff=None, vis=False, return_feat = False):
        ...
        # loss_bchwd = self.loss_decoder(loss_pred)
        ...
        # loss_patch = self.patchify(loss_bchwd).mean(dim=2, keepdim=False)
    ...
    def forward_learning_loss(self, loss_pred, loss_target, relative=False):
        """
        loss_pred: [N, L, 1]
        loss_target: [N, L]
        """
