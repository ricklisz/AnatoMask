...
def main_pt():
    ...
    # error_indices = [92 ,93 ,100 ,101 ,106]
    #
    # print('More details:')
    # print()
    # for name, param in model.named_parameters():
    #     if any(param is params_list[idx] for idx in error_indices):
    #         print(name)
    #
    # model.load_state_dict(torch.load('/home/yl_li/STUNet/nnUNet-1.7.1/nnUNet_results/STUNet_B_head.pt')['network_weights'])
    # print(model)
    ...
    output_folder = '/home/yl_li/STUNet/nnUNet-1.7.1/nnUNet_results/Dataset501_Total/Pretraining/' + model_name +'_HPM'
    ...
    # preprocessed_dataset_folder = '/scr/yl_li/segmentation_data/nnUNet_preprocessed/Dataset666_Big/3d_fullres_total_spacing'
    ...
    # plans = load_json('/scr/yl_li/segmentation_data/nnUNet_preprocessed/Dataset666_Big/nnUNetPlans.json')
    #
    ...
    # configuration_manager = plans_manager.get_configuration('3d_fullres_total_spacing')
    ...
    iters_val = len(dataset_val) // global_batch_size
    ...
    mt_gen_val = LimitedLenWrapper(iters_val, data_loader=dl_val, transform=tr_transforms,
                                     num_processes=allowed_num_processes, num_cached=6, seeds=None,
                                     pin_memory= True, wait_time=0.02)
    ...
    if pretrain_total:
        ...
        dataset_tr2 = nnUNetDataset(preprocessed_dataset_folder, tr_keys,
                                   folder_with_segs_from_previous_stage=None,
                                   num_images_properties_loading_threshold=0)
        dataset_val2 = nnUNetDataset(preprocessed_dataset_folder, val_keys,
                                    folder_with_segs_from_previous_stage=None,
                                    num_images_properties_loading_threshold=0)

        dataset_json = load_json('/scr/yl_li/segmentation_data/nnUNet_preprocessed/Dataset501_Total/dataset.json')
        plans = load_json('/scr/yl_li/segmentation_data/nnUNet_preprocessed/Dataset501_Total/nnUNetPlans.json')
        plans_manager2 = PlansManager(plans)
        configuration_manager2 = plans_manager2.get_configuration('3d_fullres')

        label_manager2 = plans_manager2.get_label_manager(dataset_json)

        patch_size = configuration_manager2.patch_size
        dim = len(patch_size)
        ...
        dl_tr2 = nnUNetDataLoader3D(dataset_tr2, batch_size,
                                   initial_patch_size,
                                   configuration_manager2.patch_size,
                                   label_manager2,
                                   oversample_foreground_percent=0.33,
                                   sampling_probabilities=None, pad_sides=None)
        dl_val2 = nnUNetDataLoader3D(dataset_val2, batch_size,
                                   initial_patch_size,
                                   configuration_manager2.patch_size,
                                   label_manager2,
                                   oversample_foreground_percent=0.33,
        ...
        iters_train = len(dataset_tr2)// global_batch_size
        iters_val = len(dataset_val2)// global_batch_size
        deep_supervision_scales = list(list(i) for i in 1 / np.cumprod(np.vstack(
            configuration_manager.pool_op_kernel_sizes), axis=0))[:-1]
        mirror_axes = (0, 1, 2)

        tr_transforms2 = get_training_transforms(
            patch_size, rotation_for_DA, deep_supervision_scales, mirror_axes, False,
            order_resampling_data=3, order_resampling_seg=1,
            use_mask_for_norm=configuration_manager.use_mask_for_norm,
            is_cascaded=False, foreground_labels=label_manager2.foreground_labels,
            regions=label_manager2.foreground_regions if label_manager2.has_regions else None,
            ignore_label=label_manager2.ignore_label)

        allowed_num_processes = get_allowed_n_proc_DA()

        mt_gen_train2 = LimitedLenWrapper(iters_train, data_loader=dl_tr2, transform=tr_transforms2,
                                         num_processes=allowed_num_processes, num_cached=6, seeds=None,
                                         pin_memory=True, wait_time=0.02)
        mt_gen_val2 = LimitedLenWrapper(iters_val, data_loader=dl_val2, transform=tr_transforms2,
                                         num_processes=allowed_num_processes, num_cached=6, seeds=None,
                                         pin_memory=True, wait_time=0.02)

        iters_train = (len(dataset_tr2) + len(dataset_tr)) // global_batch_size
        iters_val = (len(dataset_val2) + len(dataset_val)) // global_batch_size
        ...
        for idx in range(iters_train):
            ...
            if AMP:
                ...
                with torch.cuda.amp.autocast():
                    ...
                    mask = model_ema.ema.generate_mask(loss_pred, guide=guide, epoch=i, total_epoch=epoch)
                    # print('check inp, ', inp.shape, torch.isnan(inp).any(), torch.isinf(inp).any())
                    # print('check mask, ', mask.shape, torch.isnan(mask).any(), torch.isinf(mask).any())
