...
# Set requires_grad to False for all parameters in the target network
...
net = nn.Sequential(
    ...
    nn.Flatten(),
    nn.Linear(512, 1024),  # Replace stunet_output_dimension
    nn.ReLU(),
    nn.Linear(1024, 256)
        )

model = BYOL(net = net , hidden_layer=-1).to(device)
...
for i in range(epoch):
    ...
    for idx in range(iters_train):
        ...
        if AMP:
            ...
            #
            # stunet_features1 = stunet_head(augmented_image1)
            # stunet_features2 = stunet_head(augmented_image2)
